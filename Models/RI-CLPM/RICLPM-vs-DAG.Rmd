...
So, this file is just a sanity check. I'm just gonna test if we can recover the effects using a regression approach implied by the DAG (see below), and by simplifying the problem by just focusing on 1 lag.
...
get_ols_coefs <- function(ols_fit, coefs, label) {
    if(class(ols_fit) == "lmerMod") {
        ols_coefs <- fixef(ols_fit)
    } else {
...
# Long-format
Simulate data where all effects are equal at each wave
```{r}
params <- list(
    n = 1e4,
    # beta = autoregressive params
...
Fit constrained RICLPM (reference).
...
res <- rbind(
    get_ols_coefs(
        lmer(x ~ x_lag + y_lag + (1 | id), data = d_long),
        c("x_lag", "y_lag"),
        "LMM (unadjusted)"
    ),
    get_ols_coefs(
        lm(x ~ x_lag + y_lag, data = d_long),
        c("x_lag", "y_lag"),
        "OLS (unadjusted)"
    ),
    get_ols_coefs(
        lmer(x ~ x_lag + y_lag + U_x + U_y + (1 | id), data = d_long),
        c("x_lag", "y_lag"),
        "LMM (adjusted, true)"
    ),
    get_ols_coefs(
        lm(x ~ x_lag + y_lag + U_x + U_y, data = d_long),
        c("x_lag", "y_lag"),
        "OLS (adjusted, true)"
    ),
    get_ols_coefs(
        lmer(wx ~ wx_lag + wy_lag + (1 | id), data = d_long),
        c("wx_lag", "wy_lag"),
        "LMM (ws, true)"
    ),
    get_ols_coefs(
        lm(wx ~ wx_lag + wy_lag, data = d_long),
        c("wx_lag", "wy_lag"),
        "OLS (ws, true)"
    ),
    get_ols_coefs(
        lmer(wxm ~ wxm_lag + wym_lag + (1 | id), data = d_long),
        c("wxm_lag", "wym_lag"),
        "LMM (ws, mean-centered)"
    ),
    get_ols_coefs(
        lm(wxm ~ wxm_lag + wym_lag, data = d_long),
        c("wxm_lag", "wym_lag"),
        "OLS (ws, mean-centered)"
    ),
    get_ols_coefs(
        fit_RICLPM2,
        c("a", "b"),
...
- Centering using the observed subject-means is === same as adjusting for the observed subject-means. 

- The LMMs are all singular, coincidence or no variation left after including lags?
