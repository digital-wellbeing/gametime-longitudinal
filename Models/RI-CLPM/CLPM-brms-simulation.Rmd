...
compare_models <- function(fit_brms, fit_lavaan) {
    coefs_brms <- fixef(fit_brms)[5:12, ]
    coefs_lavaan <- coef(fit_lavaan)[1:8]
...
## Long format?
Can we get similar answers from a model in long format? 
```{r}
d_long <- pivot_longer(
    d,
    -id,
...
summary(fit_brms_long_centered)
```

Should the long brms models be similar to the constrained CLPM in lavaan? 
```{r}
CLPM_constrained <- '
  # Estimate the lagged effects between the observed variables.
  x2 ~ a*x1 + b*y1 
...
Results in long format seems off? Or just difficult to compare? 