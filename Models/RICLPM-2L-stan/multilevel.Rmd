...
In this document I try to fit a multilevel RICLPM using custom stan code (based on a blavaan model), I then compare it to the multigroup lavaan + random effects meta-analysis approach.
  ...
  summarise(
    fit = list(
      update(
        ...
        refresh = 0,
        file = here(str_glue("Models/RICLPM-2L-stan/brms/brm-ma-{unique(i)}"))
      )
    )
  ) %>% 
  collect()
...
I didn't meta-analyze the random intercepts, so this is only the Stan two-level model.
```{r, echo = FALSE}
...
- Should run a simulation to check the CIs for this model, wouldn't take too long.
