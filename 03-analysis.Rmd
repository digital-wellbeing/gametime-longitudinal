...
```{r riclpm-crosslag-scatter-motivations, fig.width = 8, fig.height = 10, fig.cap = "Scatterplots of well-being (rows indicate variables and waves) on intrinsic and extrinsic motivation during the previous wave. Regression lines are GAM fitted lines and 95%CIs."}
p_tmp_1 / p_tmp_2
```
...
fit_ma %>% 
  ...
  mutate(
    out = map(
      fit, 
      ~describe_posterior(.x, rope_range = c(-.17, .17), rope_ci = 1) %>% 
        select(-Parameter)
    )
  ) %>% 
  unnest(out) %>% 
  select(x_var:Type, ROPE_low:ROPE_Percentage) %>% 
  mutate(ROPE_Percentage = percent(ROPE_Percentage, .1)) %>% 
  kable(
    caption = "Play-->SPANE path ROPE.")
```
