...
cluster_library(cluster, c("brms", "purrr", "cmdstanr", "rstan"))
...
get_ma_post <- function(x) {
  coef(x, summary = FALSE) %>%
    ...
    as_tibble() %>% 
    describe_posterior(centrality = "mean", ci = 0.95) %>%
    rename(Game = Parameter)
}
get_ma_post <- memoise(get_ma_post, cache = cd)

# Table of all parameters from meta-analyses
ma_pars <- fit_ma %>%
  mutate(out = map(fit, get_ma_post)) %>%
  select(-fit, -out, -data, -name) %>%
  unnest(out2) %>%
...
ma_pars %>% 
  mutate(
    pd = percent(pd, .1),
    across(c(model, Type, y_var, x_var, Parameter, Game), factor),
    across(where(is.numeric), ~format(round(.x, 2), nsmall = 2)),
...
Order <- distinct(fit_ma, x_var, y_var, Parameter) %>%
  ...
  mutate(
    ...
    y_lab = case_when(
      ...
      y_var == "AN" ~ "Negative~affect",
      y_var == "AP" ~ "Positive~affect"
    ),
...
We also analyze positive and negative affect separately.

```{r}
#| fig.height: 3
#| fig.cap: >
#|  Forest plots of meta-analytic raw cross-lagged regression coefficients from models examining the effects of play time on negative and positive affect. Shaded curves indicate approximate posterior densities, which are numerically summarised by means and 95%CIs in the right margin. Numbers below average effects indicate posterior probabilities of direction.
fp_1 <- forest_plot(
  object = fit_ma_sum,
  type = "Unstandardized",
  x_var = "Hours",
...
x <- fit_ma %>% 
  ...
  rename(
    b = b_Intercept,
    sd = sd_Game__Intercept
  ) %>% 
  select(-starts_with("."))

# Calculate 90% heterogeneity intervals
x <- x %>% 
  mutate(
    lower = b - abs(qnorm(.025))*sd,
    upper = b + abs(qnorm(.0975))*sd
  )

# Summarize
x %>% 
  group_by(y_var) %>% 
  summarise(across(c(b, sd, lower, upper), median))
