...
```{r variables-over-time, fig.width = 8, fig.height = 8}
tmp <- d %>%
  select(
    Game, Wave, pid,
    Hours, CSAS, SPANE,
...
```{r riclpm-data-figure, fig.width = 8, fig.height = 6}
Order <- tibble(
...
```{r riclpm-data-figure-2, fig.width = 8, fig.height = 10}
p_tmp_1 / p_tmp_2
```
...
```{r riclpm-covariance-parameters, fig.height = 5, fig.width = 8}
pars_riclpm %>%
  rename(Predictor = x_var, Outcome = y_var) %>% 
  filter(Outcome == "SPANE") %>% 
  filter(Type == "Unstandardized", str_detect(Parameter, " ~~ ")) %>% 
...
```{r forest-hours-wb, fig.width = 8, fig.height = 5}
...
```{r forest-motivations-wb, fig.width = 8, fig.height = 9}
fp_1 <- forest_plot(
  object = fit_ma_sum,
  type = "Unstandardized", 
  x_var = "Intrinsic",
...
```{r time-subjective-objective-histogram, fig.height = 7, fig.width = 8}
tmp <- d %>% 
...
```{r time-subjective-objective-scatterplot, fig.height = 7, fig.width = 8}
d %>% 
  drop_na(Hours, hours_est) %>% 
  filter(Hours < 3, hours_est < 3) %>% 
  ...
  facet_grid(Wave~Game, margins = TRUE) +
...
```{r forest-subjective-wb, fig.width = 8, fig.height = 5}
fp_1 <- forest_plot(
  object = fit_ma_sum,
  type = "Unstandardized", 
  x_var = "hours_est",
