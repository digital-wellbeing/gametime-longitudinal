...
d %>% 
  ...
  as_gt() %>% 
  tab_header(
    title = "Counts of people responding to survey at X waves.",
...
d %>% 
  ...
  tab_header(
    title = "Counts of people responding to survey at X waves.",
    subtitle = "Inclusive of everyone after removing those who answered 0 surveys."
  )
...
We remove at this point all from the telemetry who didn't have any survey data

```{r}
d_t <- d_t %>% 
  mutate(Has_survey = pid %in% unique(d$pid))

d_t %>% 
  distinct(Game, pid, Has_survey) %>% 
  select(-pid) %>% 
  tbl_summary(by = Game) %>% 
  add_overall() %>% 
  as_gt() %>% 
  tab_header(title = "Number of people in telemetry with survey by game")

d_t <- filter(d_t, Has_survey) %>% 
  select(-Has_survey)
```

Deal with potentially overlapping sessions
```{r}
...
NB: we remove all negative sessions before merging sessions.
...
if(file.exists(data_path)) {
  ...
  d_t <- d_t %>%
    ...
    collect() %>% 
    ungroup() 
  write_rds(d_t, file = here("Data/session-overlap-merged.rds"))
}
```
...
d %>% 
  ...
  tab_header(
    title = "Counts of people with play at X waves",
    subtitle = "Inclusive of everyone with survey responses and telemetry"
  )
