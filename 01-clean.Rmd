...
Then look at individual sessions' durations. 

```{r session-duration-histograms, fig.cap = "Histograms of individual session durations (after taking out bad sessions and merging overlapping sessions)."}
d_t %>%
  ggplot(aes(Hours)) +
  geom_histogram() +
  scale_x_continuous(
...
d %>%
  ...
  tbl_summary(by = Game) %>%
  add_overall() %>%
  as_kable_extra(caption = "Counts of people with play at X waves")
